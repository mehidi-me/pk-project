            ...
            <div class="col-lg-6">
                <div class="card custom--card @if ($isHoliday && !$general->holiday_withdraw) card countdown-card @endif">
                    <div class="card-body">
                        ...
                        @else
                            <form id="wSubmitForm" action="{{ route('user.withdraw.money') }}" method="post">
                                @csrf
                                <div class="form-group">
                                    ...
                                    <select class="form--control" id="method-code" name="method_code" required>
                                       ...
                                       <p class="mt-2">your current withdrawable balance is: {{number_format(auth()->user()->interest_wallet,2,'.','')}} {{ $general->cur_text }}</p>
                                </div>
...
@push('script')
    <script>
       ...
       "max" : parseFloat(resource.max_limit).toFixed(2),       
       "min" : parseFloat(resource.min_limit).toFixed(2)
    });
            ...
            $('input[name=amount]').on('input', function() {
             ...
             $('#wSubmitForm').submit(function(event) {
                var amount = parseFloat($('input[name=amount]').val());
                var useAmountB = {{ auth()->user()->interest_wallet }};
                if (amount > parseFloat(useAmountB)) {

                    event.preventDefault();
                    iziToast.error({
                                message: "You do not have suffcient balance for withdraw",
                                position: "topRight"
                            });
